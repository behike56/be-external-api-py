...
VScodeとそのプラグインによって、コンテナ内に接続し開発ができる。
GitHubへのコミットも可能。

## 開発コンテナの変更方法

ライブラリの追加や更新といった場合の開発環境の変更

1. 必要な変更を行う。(poetry add library, poetry lock　など)
2. コンテナ＆イメージの再作成

## 開発コンテナが作成された後

最初にコンテナに接続した時、プラグインのインストールが行われる。
「ウィンドウの再読み込み」を何回か行う必要がある。
